...
class FinansPesinatIslem:
        ...
        try:
            ...
            self.data.update_insert(
                ...
                insert into OdemelerTB (
                    Tarih,MusteriID,SiparisNo,FinansOdemeTurID,Tutar,Masraf
                )
                values
                (?,?,?,?,?,?)
                """,
                (
                    item['tarih'],item['musteri_id'],item['siparis_no'],1,
                    item['tutar'],item['masraf']
                )
            )

            self.__siparisKontrol(item)

            mail_konu = f" {item['musteri_adi']} {item['siparis_no']} {item['tutar']} / {item['masraf']} "
    ...
    def __siparisKontrol(self,item): #bekleyendeki siparişin peşinatını tam ödediği takdirde uretime geciyor.bunun  kontrolunu saglamak için
